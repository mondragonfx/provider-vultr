...
type MetalServerInitParameters struct {
...
type MetalServerObservation struct {
...
type MetalServerParameters struct {
...
// MetalServerSpec defines the desired state of MetalServer
type MetalServerSpec struct {
	v2.ManagedResourceSpec `json:",inline"`
	ForProvider            MetalServerParameters `json:"forProvider"`
	...
	// autoscaler.
	InitProvider MetalServerInitParameters `json:"initProvider,omitempty"`
}

// MetalServerStatus defines the observed state of MetalServer.
type MetalServerStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        MetalServerObservation `json:"atProvider,omitempty"`
}
...
// MetalServer is the Schema for the MetalServers API. Provides a Vultr bare metal server resource. This can be used to create, read, modify, and delete bare metal servers on your Vultr account.
...
type MetalServer struct {
	...
	Spec   MetalServerSpec   `json:"spec"`
	Status MetalServerStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// MetalServerList contains a list of MetalServers
type MetalServerList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []MetalServer `json:"items"`
}

// Repository type metadata.
var (
	MetalServer_Kind             = "MetalServer"
	MetalServer_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: MetalServer_Kind}.String()
	MetalServer_KindAPIVersion   = MetalServer_Kind + "." + CRDGroupVersion.String()
	MetalServer_GroupVersionKind = CRDGroupVersion.WithKind(MetalServer_Kind)
)

func init() {
	SchemeBuilder.Register(&MetalServer{}, &MetalServerList{})
}
